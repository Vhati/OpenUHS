 ...
 * <p>
 * "*.PUHS" files are generated by the UHS Source Editor v4.
 */
public class Proto4xUHSParser {
	 ...
	 * @see #parse4xFormat(Proto4xUHSParseContext)
	 */
	public UHSRootNode parseFile( File f ) throws IOException, UHSParseException {
		...
		try {
			...
			br = new BufferedReader( new InputStreamReader( is, asciiDecoder ) );

			String line;
			if ( (line=br.readLine()) != null || !"proto_UHS".equals( line ) ) {
		...
		UHSRootNode rootNode = parse4xFormat( context );
		return rootNode;
	}
	 ...
	 * <p>
	 * These have not been seen in the wild.
	 * <p>
	 * They're unencrypted and intended to be submitted by authors directly
	 * to the publisher. Oddly, UHS Source Editor 4.x (and presumably this
	 * format) doesn't fully support the features in the UHS 9x formats.
	 * <p>
	 * Each node is given an arbitrary id on its START
	 ...
	 * </pre></blockquote>
	 * <p>
	 * The root node would normally contain up to two children.
	 * <ul>
	 * <li>A 'proto_subject', containing all the subjects, hints, etc., that
	 * users care about. Official readers will begin inside this node.</li>
	 * <li>A 'proto_info', mentioning the author, publisher, etc.</li>
	 * </ul>
	 * <p>
	 * Official readers merge the content of the auxiliary nodes to appear
	 * together as "Credits and File Information".
	 *
	 * @param context  the parse context
	 * @return the root of a tree of nodes
	 * @see #buildNodes(Proto4xUHSParseContext, UHSNode, int)
	 */
	public UHSRootNode parse4xFormat( Proto4xUHSParseContext context ) throws UHSParseException {
		try {
			...
			int index = 1;
			// Build the master proto_subject node.
			index += buildNodes( context, rootNode, index );

			// Build auxiliary nodes: proto_credit, proto_info.
	 ...
	 * <p>
	 * The # character is the main escape char and is written <b>##</b>.
	 * <br>
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * TODO: This needs to be delegated to node content decorators.
	 * <br>
	 * <ul>
	 ...
	 * </pre></blockquote>
	 * <p>
	 * Paths point to png images (spaces are replaced with "?").
	 * <p>
	 * There is no comparable gifa structure.
	 * <p>
	 * To create this in the UHS Source Editor...
	 * <ul>
