 ...
 * <p>
 * Each node has an optional id. The id is what the root node uses
 ...
 * parsing, the id's value is arbitrary but must be unique.
 * <p>
 * Each node has string content. Subclasses may offer additional
 * methods for images or audio.
 * <p>
 * A node may optionally act as a group, containing nested
 * child nodes. A group can be visited, causing its own content to
 * appear as a title, with its children's content listed below.
 * The revealed amount attribute tracks the nth visible child.
 * <p>
 * A node may optionally act as a hyperlink to another node.
 * A link points to an id, resolved by the root node upon clicking.
 * <p>
 * Note: Link nodes cannot be group nodes, and vice versa.
 * Setting a non-negative link target will remove all children.
 * Adding children will reset the link target.
 */
public class UHSNode {
	 ...
	 * <p>
	 * This will be spliced into a message generated by printNode().
	 *
